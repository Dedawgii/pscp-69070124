...
| OJ submission ID, if submitted | 	555735 |
| OJ status | Pass |
...
```text
เนื่องจากตอนแรกผมทำด้วยตนเองแล้วใช้ if else มันทำให้ตัวโค้ดมีความยาวประมาณหนึ่ง ผมจึงถาม AI ถึงสูตรที่มาจากสูตรเปรียบเทียบสมการอุณหภูมิที่เทียบ C/5 = (F -32)/5 = (K - 273.15)/9 = (R - 491.67)/9 เพื่อจะเขียนโค้ดที่สั้นลง แล้วค่อยดึงค่าคงที่จากสมการดังกล่าวมาคำนวณแทนที่จะเช็คเงื่อไขไปมาเยอะๆ
```
...
```text
ในข้อนี้มันแค่ช่วยทำให้วิธีอื่นที่ผมอยากลองทำมันสำเร็จได้
```
...
```text
ผมได้ลองตรวจสอบว่าสมการคำนวณได้จริง ตรงสูตรเปรียบเทียบจริง ค่าคงที่แต่ละตัวถูกต้องจริง
```
...
```text
ข้อนี้AIได้แค่ช่วยค้นหาสูตรดังกล่าว เพียงแต่ผมได้เรียนีู้แนวทางแก้ไขปัญหาที่เร้วแต่สะดวกขึ้นจากเดิม
```
